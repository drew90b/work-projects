...
- **daily-notes/** — One file per day in `YYYY/MM/YYYY-MM-DD.md` format. Includes the morning maintenance update sent to operators.
...
- **reference/** — Static reference material (SOPs, contacts, specs).
...
5. If unclear → `inbox/` for manual review

## Daily Note Format

Each daily note should include:
- Date header
- **Maintenance Update** section (the message sent to operators each morning)
- **Inbox Processed** section (what was moved where)
- **Work Done** section (tasks completed)
- **Notes** section (anything else noteworthy)
